...
import { formatNumber, formatDate, networks } from "./utils/format.js";
...
const xBONDContractInfo = ({ contract, web3 }) => {
  ...
  const { tokenName, shareName, blockExplorerUrls } = networks["pulsechain"] || { tokenName: "PLSX", shareName: "xBOND", blockExplorerUrls: [""] };
  ...
  const fetchInfo = async (retryCount = 0) => {
    ...
    try {
      ...
      if (!contract.methods.getPLSXBackingRatio) {
        throw new Error("Method getPLSXBackingRatio not found in contract ABI");
      }
        ...
        <>
          ...
          {poolInfo.poolAddress !== "0x0" && (
            <>
              ...
              <p>
                <strong>Time Until Next Withdrawal:</strong> {withdrawalCountdown}
              </p>
              <p>
