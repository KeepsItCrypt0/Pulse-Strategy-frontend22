...
const xBONDIssue = ({ web3, contract, account }) => {
  ...
  const handleIssueShares = async () => {
    ...
    try {
      ...
      const token = await getTokenContract(web3, "pulsechain");
      ...
      else if (err.message.includes("InsufficientBalance")) errorMessage = "Insufficient PLSX balance";
  ...
  return (
    <div className="bg-white bg-opacity-90 shadow-lg rounded-lg p-6 card mt-4">
      <h2 className="text-xl font-semibold mb-4 text-purple-600">Issue xBOND</h2>
      <input
        type="number"
        value={amount}
