...
const xBONDRedeem = ({ contract, account, web3 }) => {
  ...
  const handleRedeemShares = async () => {
    ...
    try {
      ...
      if (err.message.includes("InsufficientBalance")) errorMessage = "Insufficient xBOND balance";
  ...
  return (
    <div className="bg-white bg-opacity-90 shadow-lg rounded-lg p-6 card mt-4">
      <h2 className="text-xl font-semibold mb-4 text-purple-600">Redeem xBOND</h2>
      <input
        type="number"
        value={amount}
        onChange={(e) => setAmount(e.target.value)}
        placeholder="Enter xBOND amount"
